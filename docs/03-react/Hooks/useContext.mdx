...
Context 是 React 的一個功能，它可以在前端的樹狀元件結構中的任意深度傳遞數據，而無需手動從父組件傳遞 props 到子組件。使用 Context 可以更方便地在程式中管理全域資料，例如語言設置、主題、用戶資料等。
...
使用 `createContext` 在元件外面建立一個 context object，而 `defaultValue` 不一定需要，有的話可以給這個 Context 預設值，如果 Provider 沒有給 `value`，那麼拿取 Context 資料的元件將會拿到這個預設值。
