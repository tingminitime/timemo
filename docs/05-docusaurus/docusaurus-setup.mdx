...
- `docusaurus.config.js` - Docusaurus 的配置設定檔，通常愈需要客製自己的網站，愈需要頻繁調整這邊的配置，包含網站主題、網站 icon、部署、全站 `head`、全站 `meta`、docs、blog、頂部導覽列、底部 `footer`、docusaurus 內建 plugins、第三方套件、Google analytics 等等相關設定。( [官方文件說明](https://docusaurus.io/zh-CN/docs/configuration) )
...
preset 預置設定中的 `theme.customCss` 將路徑 `./src/css/custom.css` 改為 `./src/scss/custom.scss`。
...
因為專案放在 GitHub 上，這邊我使用 GitHub 登入。
