...
比較要注意的是，Promise 的回調函式中程式碼的執行依然是 **同步函式**，直到遇到 `resolve()` 或者 `reject()`，JavaScript 引擎會發出微任務，將其結果放到微任務佇列等待執行。
...
當我們在 Promise 調用了 `resolve()`，JavaScript 引擎會發起一個**微任務**，這個任務由 `then` 的第一個參數 ( Callback ) 所執行，並且這個 Callback 的參數帶有 `resolve(result)` 的 result 結果。
